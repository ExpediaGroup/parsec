...
* [`src/parsec_web/styles.cljs`](src/parsec_web/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/parsec_web/`](src/parsec_web/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/parsec_web/core.cljs): contains the SPA entry point, `init`
* [`test/parsec_web/`](test/parsec_web/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `parsec-web` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/parsec_web/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns parsec-web.example
  (:require [parsec-web.config :as config])
